...
export default function Work({
  ...
  const { userProfile, updateWorkInfo, addWorkExperience, removeWorkExperience } = useUserFormDataStore();
  const { workInfo } = userProfile.profileData;
  ...
  const handleNewWorkChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    if (typeof value === 'object' && value !== null) {
      const { noc, title, teer } = value as any;
      setNewWork(prev => ({
        ...prev,
        nocCode: noc,
        jobTitle: title,
        ...
        {workInfo.hasWorkExperience && (
          <>
            <div className="border border-secondary-200 rounded-md p-4 mt-6 bg-secondary-50">
              <h3 className="text-lg font-medium mb-4 text-black">Add Work Experience</h3>
